...
export function startGame(params: GameBackend | TournamentBracketBackend): void {
    if (isGame(params)) {
        // Local or 1v1 game
        runGame(params);
...
function runGame(game: GameBackend, tournamentId?: string) {
    game.status = "running";
    broadcastGameStatus(game, tournamentId);

    // Example: Start the game loop (replace with your actual logic)
    const gameInstance = new Game(game); // Pass the game object to your Game class
    ...
    gameInstance.onEnd = (winner: ConnectedUser, loser: ConnectedUser) => {
        ...
        if (tournamentId) {
            handleTournamentProgression(tournamentId, game, winner, loser);
        }
    };
}
...
function handleTournamentProgression(tournamentId: string, finishedGame: GameBackend, winner: ConnectedUser, loser: ConnectedUser) {
    ...
    if (bracket.game3.players[0] && bracket.game3.players[1] && bracket.game3.status === "waiting") {
        runGame(bracket.game3, tournamentId);
    }
    if (bracket.game4.players[0] && bracket.game4.players[1] && bracket.game4.status === "waiting") {
